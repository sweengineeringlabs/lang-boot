...
This guide defines versioning standards, release processes, and version management best practices for libraries and frameworks.
...
### Deprecated
- Function `old_method()` - use `new_method()` instead

### Removed
(none)

### Fixed
- Fix crash when input is empty (#123)
...
- Incorrect error message (#789)

## [1.3.1] - 2024-01-10

### Fixed
- Critical security vulnerability in validator (CVE-2024-1234)

## [1.3.0] - 2024-01-05
...

[Unreleased]: https://github.com/user/repo/compare/v1.4.0...HEAD
[1.4.0]: https://github.com/user/repo/compare/v1.3.1...v1.4.0
...
- [ ] Create release branch (e.g., `release/1.4.0`)
...
- [ ] Publish to package registry (crates.io, npm, PyPI, etc.)
- [ ] Push Git tag
...
```toml
# rustboot-core/Cargo.toml
[package]
version = "2.1.0"

# rustboot-utils/Cargo.toml
[package]
version = "1.8.3"
```

**When to use which**:
- **Synchronized**: Framework crates released together, tight coupling
...
# Edit Cargo.toml in root

# Update all crate versions that use workspace.version
# They'll automatically pick up the new version
...
# Update Cargo.lock
cargo update -p your-crate-name
```
...
```
release/1.4.0    ✅ Good - clear version
release-1.4.0    ✅ Good - alternative format
v1.4.0           ❌ Bad - confusing with tags
release/v1.4.0   ❌ Bad - redundant 'v'
1.4.0            ❌ Bad - unclear purpose
```

### Coordinating with CHANGELOG

**Every version should**:
1. Have entry in CHANGELOG.md
2. Have corresponding Git tag
3. Have GitHub release (optional but recommended)
4. Match version in package manifest

**Workflow**:
```bash
# 1. Update CHANGELOG
vim CHANGELOG.md
# Move items from [Unreleased] to [1.4.0] - 2024-01-15
...
cargo release patch  # Bumps patch version
cargo release minor  # Bumps minor version
cargo release major  # Bumps major version
```

**releasy**:
```bash
# Generate CHANGELOG from git commits
cargo install releasy
releasy changelog
```
...
6 weeks  6 weeks  6 weeks  for breaking
```
...
#### Pattern 3: LTS (Long-Term Support)

```
v2.4.0 (LTS - supported until 2026)
  ├─ v2.4.1
  ├─ v2.4.2
  └─ v2.4.3
...
**If published to registry** (crates.io, npm):
...
- [Rust Packaging vs Java](../../4-development/guide/rust-packaging-vs-java.md) - Rust-specific versioning
